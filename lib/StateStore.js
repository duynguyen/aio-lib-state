...
class StateStore {
  ...
  /**
   * Retrieves all resources for a given query spec.
   * If no result, return an empty array.
   *
   * @param {string} spec SQL query statement
   * @returns {Promise<StateStoreQueryReturnValue>} get response holding an array of values
   * @memberof StateStore
   */
  async query (spec) {
    // validateKey(key, { key })
    logger.debug(`query '${spec}'`)
    return this._query(spec)
  }
  ...
  async _query (spec) { throwNotImplemented('_query') }
